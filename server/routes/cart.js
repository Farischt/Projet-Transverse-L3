...
router.post("/:_id", verifyAuth, async (req, res) => {
  ...
  const cart = req.session.cart.items.find(
    (element) => element._id === req.params._id
  );
  if (cart)
    return res
      .status(400)
      .send(
        "Item is already in cart, delete it, or change quantity in the cart"
      );
...
router.delete("/:_id", verifyAuth, async (req, res) => {
  ...
  // We then check if the item is in the cart.
  const itemInCart = req.session.cart.items.findIndex(
    (element) => element._id === req.params._id
  );
